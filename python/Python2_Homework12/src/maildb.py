...
#        return result[0]
    ...
    curs.execute("INSERT INTO message (msgDate, msgRecipientName, msgRecipientAddress,msgText) VALUES (%s, %s, %s, %s)", (dt, name, email, text))
    conn.commit()
    curs.execute("SELECT msgID FROM message WHERE msgRecipientName LIKE \%%s\% AND msgRecipientAddress LIKE \%%s\% AND msgDate=%s", (name,email,date))
